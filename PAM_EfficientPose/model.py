...
def build_EfficientPose(phi,
    ...
    """
    (<tf.Tensor 'fpn_cells/cell_2/fnode3/op_after_combine8/bn/FusedBatchNormV3:0' shape=(?, 64, 64, 64) dtype=float32>, 
    <tf.Tensor 'fpn_cells/cell_2/fnode2/op_after_combine7/bn/FusedBatchNormV3:0' shape=(?, 32, 32, 64) dtype=float32>, 
    <tf.Tensor 'fpn_cells/cell_2/fnode1/op_after_combine6/bn/FusedBatchNormV3:0' shape=(?, 16, 16, 64) dtype=float32>, 
    <tf.Tensor 'fpn_cells/cell_2/fnode0/op_after_combine5/bn/FusedBatchNormV3:0' shape=(?, 8, 8, 64) dtype=float32>, 
    <tf.Tensor 'fpn_cells/cell_2/fnode7/op_after_combine12/bn/FusedBatchNormV3:0' shape=(?, 4, 4, 64) dtype=float32>)
    """
...
def build_Direct_Concate_EfficientPose(phi,
                        num_classes = 8,
                        num_anchors = 9,
                        freeze_bn = False,
                        score_threshold = 0.5,
    ...
    """
    (<tf.Tensor '0/concat:0' shape=(?, 64, 64, 128) dtype=float32>, 
    <tf.Tensor '1/concat:0' shape=(?, 32, 32, 128) dtype=float32>, 
    <tf.Tensor '2/concat:0' shape=(?, 16, 16, 128) dtype=float32>, 
    <tf.Tensor '3/concat:0' shape=(?, 8, 8, 128) dtype=float32>, 
    <tf.Tensor '4/concat:0' shape=(?, 4, 4, 128) dtype=float32>)
    """
    ...
    #input layers
    # image_input = layers.Input(input_shape)
    # image_input_r = layers.Input(input_shape)
    camera_parameters_input = layers.Input((6,)) #camera parameters and image scale for calculating the translation vector from 2D x-, y-coordinates
    # camera_parameters_input_r = layers.Input((6,))

    #build EfficientNet backbone
    # backbone_feature_maps = backbone_class(input_tensor = image_input, freeze_bn = freeze_bn)
    # backbone_feature_maps_r = backbone_class(input_tensor = image_input_r, freeze_bn = freeze_bn)
    
    #build BiFPN
    # fpn_feature_maps = build_BiFPN(backbone_feature_maps, bifpn_depth, bifpn_width, freeze_bn)
    # fpn_feature_maps_r = build_BiFPN(backbone_feature_maps_r, bifpn_depth, bifpn_width, freeze_bn)
    ...
    # concate feature map
    input_rois = [fpn_feature_maps, fpn_feature_maps_r]
    # feature_map_list = list(fpn_feature_maps)
    # feature_map_list_r =list(fpn_feature_maps_r)
    final_feature_maps = build_concat_layers(fpn_feature_maps,fpn_feature_maps_r)
    
    # final_feature_maps = tf.concat([feature_map_list,feature_map_list_r],-1) 

    # final_feature_maps = tf.concat([feature_map_list,feature_map_list_r],-1) 
        
    # final_feature_maps = feature_fusion(fusion_method="concat",inputs = input_rois, input_weights=[1,1])
    ...
    #get the EfficientPose model for training without NMS and the rotation and translation output combined in the transformation output because of the loss calculation
    efficientpose_train = models.Model(inputs = [image_input, image_input_r, camera_parameters_input], outputs = [classification, bbox_regression, transformation], name = 'efficientpose')
    ...
    efficientpose_prediction = models.Model(inputs = [image_input, image_input_r, camera_parameters_input], outputs = filtered_detections, name = 'efficientpose_prediction')
...
def build_PAM_EfficientPose(phi,
                        num_classes = 8,
                        num_anchors = 9,
                        freeze_bn = False,
                        score_threshold = 0.5,
    ...
    # feature_map_list = list(fpn_feature_maps)
    # feature_map_list_r =list(fpn_feature_maps_r)
    final_feature_maps = build_weighted_concat_layers(fpn_feature_maps,fpn_feature_maps_r)
...
def prepare_feature_maps_for_BiFPN(C3, C4, C5, num_channels, freeze_bn):
    ...
    P3_in = BatchNormalization(freeze=freeze_bn, momentum=MOMENTUM, epsilon=EPSILON)(P3_in)
    
    P4_in = C4
    P4_in_1 = layers.Conv2D(num_channels, kernel_size=1, padding='same')(P4_in)
    P4_in_1 = BatchNormalization(freeze=freeze_bn, momentum=MOMENTUM, epsilon=EPSILON)(P4_in_1)
    P4_in_2 = layers.Conv2D(num_channels, kernel_size=1, padding='same')(P4_in)
    P4_in_2 = BatchNormalization(freeze=freeze_bn, momentum=MOMENTUM, epsilon=EPSILON)(P4_in_2)
    
    P5_in = C5
    P5_in_1 = layers.Conv2D(num_channels, kernel_size=1, padding='same')(P5_in)
    P5_in_1 = BatchNormalization(freeze=freeze_bn, momentum=MOMENTUM, epsilon=EPSILON)(P5_in_1)
    P5_in_2 = layers.Conv2D(num_channels, kernel_size=1, padding='same')(P5_in)
    P5_in_2 = BatchNormalization(freeze=freeze_bn, momentum=MOMENTUM, epsilon=EPSILON)(P5_in_2)
    
    P6_in = layers.Conv2D(num_channels, kernel_size=1, padding='same')(C5)
    P6_in = BatchNormalization(freeze=freeze_bn, momentum=MOMENTUM, epsilon=EPSILON)(P6_in)
...
def single_BiFPN_merge_step(feature_map_other_level, feature_maps_current_level, upsampling, num_channels, idx_BiFPN_layer, node_idx, op_idx):
    ...
    merged_feature_map = wBiFPNAdd()(feature_maps_current_level + [feature_map_resampled])
...
def SeparableConvBlock(num_channels, kernel_size, strides, freeze_bn = False):
    ...
    """
    f1 = layers.SeparableConv2D(num_channels, kernel_size = kernel_size, strides = strides, padding = 'same', use_bias = True, )
    f2 = BatchNormalization(freeze = freeze_bn, momentum = MOMENTUM, epsilon = EPSILON)
...
def apply_subnets_to_get_roi(box_net, fpn_feature_maps, image_input, input_size, anchor_parameters):
    """
    ...
    return  bbox_regression, bboxes, croped_feature_map
...
def build_concat_layers(left,right):
    """
    Concat layer by layer, for each input, they are tuple which contain 5 scales features, 
    we concat left and right first in different scale and then make it into new tuple

    Args:
        left and right feature maps
    Returns:
        concated feature map in different scales       
    """
    layer_0 = layers.Concatenate(axis=-1)([left[0],right[0]])
    layer_1 = layers.Concatenate(axis=-1)([left[1],right[1]])
    layer_2 = layers.Concatenate(axis=-1)([left[2],right[2]])
    layer_3 = layers.Concatenate(axis=-1)([left[3],right[3]])
    layer_4 = layers.Concatenate(axis=-1)([left[4],right[4]])
...
class PAM(models.Model):
    def __init__(self, channels, **kwargs):
        ...
        self.b1 = layers.Conv2D(channels, kernel_size=1, strides=1, padding='same', use_bias=True)
        self.b2 = layers.Conv2D(channels, kernel_size=1, strides=1, padding='same', use_bias=True)
        self.b3 = layers.Conv2D(channels, kernel_size=1, strides=1, padding='same', use_bias=True)
        self.softmax = layers.Softmax(axis=-1)
        self.rb = ResB(64)
        self.fusion = layers.Conv2D(channels, kernel_size=1, strides=1, padding='same', use_bias=True)

    def call(self, x_left, x_right, **kwargs):
        b, h, w, c = x_left.shape
        ...
        ### M_{right_to_left}
        Q = self.b1(buffer_left)                                                                    # B * H * W * C
        S = tf.transpose(self.b2(buffer_right),perm=[0, 1, 3, 2])                                   # B * H * C * W
        score = tf.matmul(tf.reshape(Q,[-1, w, c]),tf.reshape(S,[-1, c, w]))                       # (B*H) * W * W
        M_right_to_left = self.softmax(score)

        ### M_{left_to_right}
        Q = self.b1(buffer_right)                                                                  # B * H * W * C
        S = tf.transpose(self.b2(buffer_left),perm=[0, 1, 3, 2])                                   # B * H * C * W
        score = tf.matmul(tf.reshape(Q,[-1, w, c]),tf.reshape(S,[-1, c, w]))                       # (B*H) * W * W
        M_left_to_right = self.softmax(score)

        ### valid masks
        V_left_to_right = tf.reduce_sum(M_left_to_right, axis=1) > 0.1
        V_left_to_right = tf.reshape(V_left_to_right,[tf.shape(x_left)[0], 1, h, w])                                  #  B * 1 * H * W
        V_left_to_right = tf.cast(V_left_to_right,tf.float32)

        ### fusion
        buffer = tf.reshape(self.b3(x_right),[-1, w, c])                                            # (B*H) * W * C
        buffer = tf.transpose(tf.reshape(tf.matmul(M_right_to_left, buffer),[tf.shape(x_left)[0], h, w, c]),perm=[0,3,1,2])  #  B * C * H * W
        x_left = tf.transpose(x_left,perm=[0,3,1,2])
        out = self.fusion(tf.transpose(layers.Concatenate(axis=1)([buffer, x_left, V_left_to_right]),perm=[0,2,3,1])) 
...
class ResB(models.Model):
    def __init__(self, channels, **kwargs):
        super(ResB, self).__init__(**kwargs)
        self.r1 = layers.Conv2D(channels, kernel_size=3, strides=1, padding='same', use_bias=False)
        self.r2 = layers.LeakyReLU(0.1)
        self.r3 = layers.Conv2D(channels, kernel_size=3, strides=1, padding='same', use_bias=False)
...
def build_weighted_concat_layers(left,right):
    """
    Concat layer by layer, but calculate cosine similarity for each input, they are tuple which contain 5 scales features, 
    we concat left and right first in different scale and then make it into new tuple
    ...
    Returns:
        concated feature map in different scales       
    """
    stereo_attention_net = PAM(channels=64)

    layer_0 = stereo_attention_net(left[0],right[0])
    layer_1 = stereo_attention_net(left[1],right[1])
    layer_2 = stereo_attention_net(left[2],right[2])
    layer_3 = stereo_attention_net(left[3],right[3])
    layer_4 = stereo_attention_net(left[4],right[4])
