...
class LineModGenerator(Generator):
    ...
    def __init__(self, 
        ...
        Stereo setup:
            There are some component of the code can be used by both left and right frame
            They are: model_path, object_id, and extension stuff
            NEED to change: make two gt documents as well as load right frames as 'self.xxx_xx_r'
        ...
        #Add right image folder into consideration
        self.dataset_path_r = os.path.join(self.dataset_base_path, "right")
        ...
        if not self.check_path(self.dataset_base_path) or not self.check_path(self.dataset_path) or not self.check_path(self.model_path)or not self.check_path(self.dataset_path_r):
            return None
        ...
        self.object_paths_and_ids_r = {int(subdir_r): os.path.join(self.dataset_path_r, subdir_r) for subdir_r in os.listdir(self.dataset_path_r) if os.path.isdir(os.path.join(self.dataset_path_r, subdir_r))}
        ...
        if not self.object_id in self.object_paths_and_ids:
            ...
            return None
        if not self.object_id in self.object_paths_and_ids_r:
            print("The given object id {} was not found in the dataset dir {}".format(self.object_id, self.dataset_path_r))
            return None            

        
        #get path containing the data for the given object
        self.object_path = self.object_paths_and_ids[self.object_id]
        self.object_path_r = self.object_paths_and_ids_r[self.object_id]
        ...
        self.gt_dict_r = self.parse_yaml(self.object_path_r)
        ...
        self.image_paths_r, self.mask_paths_r, self.depth_paths_r, self.annotations_r, self.infos_r = self.prepare_dataset(self.object_path_r, self.data_examples, self.gt_dict_r, self.info_dict)
    ...
    def load_image_r(self, image_index):
        ...
        """
        #load image and switch BGR to RGB
        image_r = cv2.imread(self.image_paths_r[image_index])
