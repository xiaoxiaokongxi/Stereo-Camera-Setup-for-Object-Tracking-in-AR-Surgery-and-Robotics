...
class Generator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        inputs, targets = self.compute_inputs_targets_stereo(group)
        return inputs, targets
    
    def compute_inputs_targets_stereo(self, group, debug = False):
        """
        ...
        image_group_r = self.load_image_group_r(group)
        ...
        image_group, image_group_r, annotations_group = self.random_transform_group(image_group, image_group_r, annotations_group, mask_group, camera_matrix_group)

        # perform preprocessing steps
        image_group, image_group_r, annotations_group = self.preprocess_group(image_group, image_group_r, annotations_group, camera_matrix_group)

        assert len(image_group) != 0
        assert len(image_group) == len(annotations_group)

        # compute network inputs
        inputs = self.compute_inputs(image_group, image_group_r, annotations_group)
    ...
    def load_image_group_r(self, group):

        return [self.load_image_r(image_index) for image_index in group]
    ...
    def compute_inputs(self, image_group, image_group_r, annotations_group):
        """
        ...
        batch_images_r = np.array(image_group_r).astype(np.float32)
        ...
        return [batch_images, batch_images_r, batch_camera_parameters]
    ...
    def random_transform_group(self, image_group, image_group_r, annotations_group, mask_group, camera_matrix_group):
        ...
        for index in range(len(image_group)):
            ...
            image_group[index], image_group_r[index], annotations_group[index] = self.random_transform_group_entry(image_group[index], image_group_r[index], annotations_group[index], mask_group[index], camera_matrix_group[index])

        return image_group, image_group_r, annotations_group
    
    
    def random_transform_group_entry(self, image, image_r, annotations, mask, camera_matrix, transform = None):
        ...
        if chance_no_augmentation >= self.chance_no_augmentation:
            ...
            if self.use_colorspace_augmentation:
                ...
                image_r = np.expand_dims(image_r[:, :, ::-1], axis = 0)
                #apply color space augmentation
                image = self.rand_aug(images = image)
                image_r = self.rand_aug(images = image_r)
                #squeeze dims again and flip back to BGR
                image = np.squeeze(image)[:, :, ::-1]
                image_r = np.squeeze(image_r)[:, :, ::-1]
                
            if self.use_6DoF_augmentation:
                image, image_r, annotations = self.augment_6DoF_image_and_annotations(image, image_r, annotations, mask, camera_matrix)

        return image, image_r, annotations
    
    
    def augment_6DoF_image_and_annotations(self, img, img_r, annotations, mask, camera_matrix):
        ...
        augmented_img, augmented_img_r, augmented_rotation_vector, augmented_translation_vector, augmented_bbox, still_valid_annos, is_valid_augmentation = self.augmentation_6DoF(img = img,
                                                                                                                                                img_r= img_r,
                                                                                                                                                mask = mask,
        ...
        if is_valid_augmentation:
            ...
            augmented_img_r = np.array(img_r)
            # print("complete invalid")
        
        return augmented_img, augmented_img_r, annotations
    
    
    def augmentation_6DoF(self, img, img_r, mask, rotation_matrix_annos, translation_vector_annos, angle, scale, camera_matrix, mask_values):
        ...
        augmented_img_r = cv2.warpAffine(img_r, rot_2d_mat, (width, height))
        ...
        return augmented_img, augmented_img_r, augmented_rotation_vector_annos, augmented_translation_vector_annos, augmented_bbox_annos, still_valid_annos, True
    ...
    def preprocess_group(self, image_group, image_group_r, annotations_group, camera_matrix_group):
        """
        ...
        for index in range(len(image_group)):
            ...
            image_group[index], image_group_r[index], annotations_group[index] = self.preprocess_group_entry(image_group[index],
                                                                                       image_group_r[index],
    ...
    def preprocess_group_entry(self, image, image_r,  annotations, camera_matrix):
        """
        ...
        image_r, image_scale_r = self.preprocess_image(image_r)
        assert(image_scale==image_scale_r)
    ...
    def load_image_r(self, image_index):
        """ Load an image at the image_index.
        """
        #load image and switch BGR to RGB

        raise NotImplementedError('load_r method not implemented')