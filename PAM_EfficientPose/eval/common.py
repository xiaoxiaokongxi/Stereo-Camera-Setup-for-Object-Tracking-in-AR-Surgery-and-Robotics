...
def _get_detections(generator, model, score_threshold = 0.05, max_detections = 100, save_path = None):
    ...
    for i in progressbar.progressbar(range(generator.size()), prefix='Running network: '):
        ...
        raw_image_r = generator.load_image_r(i)
        image, scale = generator.preprocess_image(raw_image.copy())
        image_r, scale_r = generator.preprocess_image(raw_image_r.copy())
        ...
        # run network
        boxes, scores, labels, rotations, translations = model.predict_on_batch([np.expand_dims(image, axis=0), np.expand_dims(image_r, axis=0), np.expand_dims(camera_input, axis=0)])[:5]
