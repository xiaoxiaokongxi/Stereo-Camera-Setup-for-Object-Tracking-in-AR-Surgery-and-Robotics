...
from model import build_EfficientPose, build_Direct_Concate_EfficientPose, build_PAM_EfficientPose
...
def main(args = None):
    ...
    # model, prediction_model, all_layers = build_Direct_Concate_EfficientPose(args.phi,
    #                                                           num_classes = num_classes,
    #                                                           num_anchors = num_anchors,
    #                                                           freeze_bn = not args.no_freeze_bn,
    #                                                           score_threshold = args.score_threshold,
    ...
    model, prediction_model, all_layers = build_PAM_EfficientPose(args.phi,
                                                              num_classes = num_classes,
                                                              num_anchors = num_anchors,
                                                              freeze_bn = not args.no_freeze_bn,
                                                              score_threshold = args.score_threshold,
...
def create_callbacks(training_model, prediction_model, validation_generator, args):
    ...
    if args.snapshots:
        ...
        checkpoint = keras.callbacks.ModelCheckpoint(os.path.join(snapshot_path, 'phi_{phi}_{dataset_type}_best_{metric}.h5'.format(phi = str(args.phi), metric = metric_to_monitor, dataset_type = args.dataset_type)),
                                                     verbose = 1,
                                                     save_weights_only = True, #used to be commentted
                                                     save_best_only = True,
